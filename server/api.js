...
export async function handleApiRequest(request, env) {
    ...
    try {
      ...
      // --- units (master data hanya Admin/Koordinator) ---
      if (r === 'units' && request.method === 'POST') {
        if (me.role !== 'admin') return json({ error: 'Forbidden' }, 403, cors);
        return await upsertUnit(request, env, cors);
      }
      if (r.startsWith('units/') && request.method === 'DELETE') {
        if (me.role !== 'admin') return json({ error: 'Forbidden' }, 403, cors);
        return await deleteUnit(env, decodeURIComponent(r.slice(6)), cors);
      }

      // --- monitoring (petugas boleh menyimpan; penghapusan hanya Admin) ---
      ...
      if (r.startsWith('monitoring/') && request.method === 'DELETE') {
        if (me.role !== 'admin') return json({ error: 'Forbidden' }, 403, cors);
        return await deleteMonitoring(env, decodeURIComponent(r.slice(11)), cors);
      }
...
async function authorize(request, env) {
  ...
  if (!m) return null;
  const row = await env.DB.prepare('SELECT username, created_at FROM sessions WHERE token=?').bind(m[1]).first();
  if (!row) return null;
  if (!row.created_at || Date.now() - Number(row.created_at) > SESSION_MAX_AGE_MS) {
...
async function login(request, env, cors) {
  ...
  const t = token(), now = Date.now();
  await env.DB.batch([
    env.DB.prepare('DELETE FROM sessions WHERE created_at<?').bind(now - SESSION_MAX_AGE_MS),
    env.DB.prepare('INSERT INTO sessions(token, username, created_at) VALUES(?,?,?)').bind(t, u.username, now)
  ]);
